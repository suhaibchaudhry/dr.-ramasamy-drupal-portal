...
//function omega_kickstart_footnote_link($var) {
//  $fn = $var['fn'];
...
function omega_kickstart_preprocess_node(&$variables) {
  ...
  if (isset($variables['content']['links']['node']['#links']['node-readmore'])){
	$variables['content']['links']['node']['#links']['node-readmore']['title'] = "View";
  }
}
/**
* Make title available to zone.tpl.php also. Currently it is only available to region.tpl.php
*/
function omega_kickstart_alpha_preprocess_zone(&$vars) {
...
function omega_kickstart_preprocess_region(&$vars) {
  $theme = alpha_get_theme();
